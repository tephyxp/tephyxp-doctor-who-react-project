...
const SearchResults = () => {
  ...
  if (!storedResults) {
    ...
    return (
      <div className="bg-cover bg-center min-h-screen flex justify-center items-center bg-fixed border-b border-b-white" style={{ backgroundImage: `url('/img/backgrounds/background-meet.png')` }}>
        <div className="grid grid-cols-4 gap-6 p-10">
          {storedResults.map((result, index) => (
            <div key={index} className="col-span-1">
              <div className="relative px-5 py-10">
                <div className="relative flex items-center justify-center mb-4">
                  <img className="w-[60.625rem] h-[28.694rem] rounded-md mb-3" src={result.image} alt={result.doctor} />
                </div>
                <div className="text-white font-lato text-2xl font-bold mb-3">{result.doctor}</div>
                <div className="text-white text-lg mb-3">{result.actor}</div>
                <div className="text-white text-lg mb-3">{result.screen_time}</div>
                <button className="mt-2 bg-buttons w-52 px-4 py-2 rounded-[40px] text-black font-bold">Ver más</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {storedResults.map((result) => (
        ...
        >
          <div className=" w-1/2 flex flex-col justify-center items-center space-y-16 text-center text-white mx-4 my-10">
            ...
            <p className="break-words font-lato text-xl py-5">
